// Задайте одномерный массив из 123 случайных чисел в диапазоне [0, 150]. Найдите количество элементов массива, значения которых лежат в отрезке [10,99].
// Пример для массива из 5, а не 123 элементов. В своём решении сделайте для 123
// [5, 18, 123, 6, 2] -> 1
// [1, 2, 3, 6, 2] -> 0
// [10, 11, 12, 13, 14] -> 5

int[] arr = new int[123];
Random rand = new Random();
int count = 0;

for (int i = 0; i < arr.Length; i++)
{
    arr[i] = rand.Next(0, 150);
    if (arr[i] >= 10 && arr[i] <= 99)
    {
        count += 1;
    }
}
Console.WriteLine(" ");
Console.WriteLine("***Ответ***");
Console.WriteLine($"Количество элементов массива [{string.Join((", "), arr)}] из 123 случайных чисел в диапазоне [0, 150], значения которых лежат в отрезке [10,99], равно {count}.");
